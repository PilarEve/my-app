...
export default function DashboardPage() {
  ...
  const FilterButton = ({ label, active, onClick, colorClass }: { label: string, active: boolean, onClick: () => void, colorClass?: string }) => (
    <button
      ...
      className={cn(
        ...
        active ? "bg-white shadow-sm border-nature-300 text-nature-900" : "bg-transparent border-transparent text-nature-500 hover:bg-nature-100"
      )}
    >
      <div className={cn("w-3 h-3 rounded-full flex items-center justify-center", active ? (colorClass || "bg-nature-700") : "bg-nature-200")}>
  ...
  return (
    <div className="h-full flex flex-col">
      {/* Header Panel */}
      <div className="bg-white border-b border-nature-200 px-6 py-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-heading font-bold text-nature-900">Dashboard de Monitoreo</h2>
          <p className="text-sm text-nature-600">Visualización en tiempo real del nivel de agua y reportes ciudadanos.</p>
        </div>
        
        {/* Key metrics */}
        <div className="flex gap-4">
          <div className="bg-nature-50 px-4 py-2 rounded-lg border border-nature-100 flex items-center gap-3">
            <div className="bg-white p-1.5 rounded shadow-sm text-nature-700"><Activity size={18} /></div>
            <div>
              <p className="text-xs text-nature-500 font-medium">Eventos</p>
              <p className="font-bold text-nature-900 leading-none">{totalEvents}</p>
            </div>
          </div>
      ...
      <div className="flex-1 flex flex-col lg:flex-row p-6 gap-6 overflow-hidden">
        ...
        <div className="w-full lg:w-80 flex flex-col gap-6 overflow-y-auto pr-2">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <p className="text-xs font-semibold text-nature-500 uppercase">Fuentes de Datos</p>
              ...
              <div className="space-y-2">
                <p className="text-xs font-semibold text-nature-500 uppercase">Nivel de Riesgo</p>
                <div className="flex flex-wrap gap-2">
          ...
          <Card className="flex-1 min-h-0 flex flex-col">
            ...
            <CardContent className="h-32 min-h-[128px] w-full pb-0 mb-4 border-b border-nature-100">
            ...
            <CardContent className="flex-1 overflow-y-auto space-y-3 pb-6">
              {dataPoints.slice().reverse().map(dp => (
                <div key={dp.id} className="p-3 bg-nature-50 rounded-lg border border-nature-100 flex gap-3 text-sm">
                  <div className={cn(
                  ...
                  <div className="flex-1">
                    <p className="font-semibold text-nature-900 capitalize flex items-center gap-1">
                      {dp.source === 'sensor' ? <Activity size={14} className="text-nature-500"/> : <Users size={14} className="text-nature-500"/>}
                      {dp.source}
                    </p>
                    {dp.description && <p className="text-nature-600 text-xs mt-1 line-clamp-2">{dp.description}</p>}
                    <p className="text-[10px] text-nature-400 mt-2">{new Date(dp.timestamp).toLocaleTimeString()}</p>
                  </div>
                </div>
              ))}
            </CardContent>
        ...
        {/* Map Container */}
        <div className="flex-1 bg-white rounded-xl shadow-sm border border-nature-200 overflow-hidden relative">
          <DynamicMap />
          
          {/* Map Legend Overlay */}
          <div className="absolute bottom-6 right-6 z-[1000] bg-white/90 backdrop-blur p-4 rounded-lg shadow-lg border border-nature-100 text-xs text-nature-700">
            <p className="font-semibold text-nature-900 mb-2">Leyenda</p>
            <div className="space-y-2">
              ...
              <div className="h-px bg-nature-200 my-2"></div>
              <div className="flex items-center gap-2"><div className="w-3 h-3 border-2 border-nature-500 rounded-sm"></div> Sensor IoT</div>
              <div className="flex items-center gap-2"><div className="w-3 h-3 border-2 border-nature-500 rounded-full"></div> Ciudadano</div>
            </div>
          </div>
        </div>
      </div>
